...
class FishingMacroGUI:
    def __init__(self, root):
        ...
        # Discord Webhook & Fruit Detection
        self.discord_webhook_url = ""
        self.discord_message = "🎉 **Hey, you got oiled?** \nI found a Devil Fruit. Go pick it up."
        self.store_fruit_color = None  # [R, G, B]
        self.fruit_color_tolerance = 30
        self.periodic_screenshot_enabled = False
        self.periodic_screenshot_interval = 10  # minutes
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                self.discord_message = data.get("discord_message", "🎉 **Hey, you got oiled?** \nI found a Devil Fruit. Go pick it up.")
                self.store_fruit_color = data.get("store_fruit_color", None)
                self.fruit_color_tolerance = data.get("fruit_color_tolerance", 30)
                self.periodic_screenshot_enabled = data.get("periodic_screenshot_enabled", False)
                self.periodic_screenshot_interval = data.get("periodic_screenshot_interval", 10)
            except:
                pass
    ...
    def save_settings(self):
        ...
        with open(self.config_file, 'w') as f:
            json.dump({
                ...
                "store_fruit_color": self.store_fruit_color,
                "fruit_color_tolerance": self.fruit_color_tolerance,
                "periodic_screenshot_enabled": self.periodic_screenshot_enabled,
                "periodic_screenshot_interval": self.periodic_screenshot_interval
            }, f, indent=4)
    ...
    def create_widgets(self):
        ...
        # Store Fruit Button Color Capture
        color_capture_frame = ttk.Frame(self.auto_store_section)
        color_capture_frame.pack(fill="x", pady=3)
        
        self.capture_color_button = ttk.Button(color_capture_frame, text="Pick Green at Store Fruit", 
                                              command=self.capture_store_fruit_color_ui, width=20)
        self.capture_color_button.pack(side="left", padx=5)
        
        self.store_fruit_color_label = ttk.Label(color_capture_frame, text="Not Set", 
                                                font=("Arial", 9), foreground="red")
        
        if self.store_fruit_color:
            self.store_fruit_color_label.config(
                text=f"RGB: {self.store_fruit_color}", 
                foreground="green"
            )
        self.store_fruit_color_label.pack(side="left", padx=5)
        ...
        webhook_frame = ttk.LabelFrame(discord_tab, text="Configuration", padding=10)
        webhook_frame.pack(fill="x", padx=5, pady=5)
        
        # Webhook URL
        url_frame = ttk.Frame(webhook_frame)
        url_frame.pack(fill="x", pady=3)
        ttk.Label(url_frame, text="Webhook URL:", width=15, anchor="w").pack(side="left", padx=5)
        self.webhook_url_var = tk.StringVar(value=self.discord_webhook_url)
        webhook_entry = ttk.Entry(url_frame, textvariable=self.webhook_url_var)
        webhook_entry.pack(side="left", padx=5, fill="x", expand=True)
        webhook_entry.bind('<Return>', lambda e: self.update_discord_settings())
        webhook_entry.bind('<FocusOut>', lambda e: self.update_discord_settings())
        
        # Message
        msg_frame = ttk.Frame(webhook_frame)
        msg_frame.pack(fill="x", pady=3)
        ttk.Label(msg_frame, text="Message:", width=15, anchor="nw").pack(side="left", padx=5)
        self.discord_msg_var = tk.StringVar(value=self.discord_message)
        # Using Entry for now (could use Text for multiline but variable binding is harder)
        msg_entry = ttk.Entry(msg_frame, textvariable=self.discord_msg_var) 
        msg_entry.pack(side="left", padx=5, fill="x", expand=True)
        msg_entry.bind('<Return>', lambda e: self.update_discord_settings())
        msg_entry.bind('<FocusOut>', lambda e: self.update_discord_settings())
        
        # Actions Frame
        actions_frame = ttk.LabelFrame(discord_tab, text="Actions", padding=10)
        actions_frame.pack(fill="x", padx=5, pady=5)
        
        test_btn = ttk.Button(actions_frame, text="Test Webhook", command=self.test_discord_webhook)
        test_btn.pack(side="left", padx=5)
        
        ttk.Label(actions_frame, text="(Sends a test message with current settings)", 
                  font=("Arial", 9), foreground="gray").pack(side="left", padx=5)

        # Periodic Updates Section
        periodic_frame = ttk.LabelFrame(discord_tab, text="Periodic Status Updates", padding=10)
        periodic_frame.pack(fill="x", padx=5, pady=5)
        
        # Enable Checkbox
        self.periodic_enabled_var = tk.BooleanVar(value=self.periodic_screenshot_enabled)
        chk = ttk.Checkbutton(periodic_frame, text="Enable Periodic Screenshots", 
                              variable=self.periodic_enabled_var,
                              command=self.update_periodic_settings)
        chk.pack(anchor="w", padx=5, pady=3)
        
        # Interval
        interval_frame = ttk.Frame(periodic_frame)
        interval_frame.pack(fill="x", pady=3)
        ttk.Label(interval_frame, text="Interval (minutes):", width=20, anchor="w").pack(side="left", padx=5)
        self.periodic_interval_var = tk.IntVar(value=self.periodic_screenshot_interval)
        interval_spin = ttk.Spinbox(interval_frame, from_=1, to=1440, increment=1, 
                                    textvariable=self.periodic_interval_var, width=10)
        interval_spin.pack(side="left", padx=5)
        interval_spin.bind('<Return>', lambda e: self.update_periodic_settings())
        interval_spin.bind('<FocusOut>', lambda e: self.update_periodic_settings())
        
        ttk.Label(interval_frame, text="min").pack(side="left", padx=2)
    ...
    def toggle_macro(self):
        ...
        if self.is_running:
            ...
            threading.Thread(target=self.periodic_screenshot_checker, daemon=True).start()
        else:
    ...
    def pre_cast(self):
        ...
        if self.auto_store_devil_fruit:
            ...
            if not self.store_fruit_point:
                ...
                # 2) Check for color match (Fruit Detection)
                fruit_detected = False
                if self.store_fruit_color:
                    print("  Checking for fruit color match...")
                    # Take screenshot of the button area (5x5 box for tolerance)
                    search_radius = 2  # +/- 2 pixels -> 5x5 area
                    search_width = (search_radius * 2) + 1
                    
                    with mss.mss() as sct:
                        monitor = {
                            "top": self.store_fruit_point['y'] - search_radius,
                            "left": self.store_fruit_point['x'] - search_radius,
                            "width": search_width,
                            "height": search_width
                        }
                        ...
                        # Search for ANY matching pixel in the 5x5 area
                        match_found = False
                        closest_color = None
                        min_diff = 999999
                        
                        # Iterate through all pixels in the 5x5 box
                        for row in range(img.shape[0]):
                            for col in range(img.shape[1]):
                                pixel = img[row, col]
                                current_pixel_rgb = (int(pixel[2]), int(pixel[1]), int(pixel[0]))
                                
                                if self.is_color_match(current_pixel_rgb, self.store_fruit_color, self.fruit_color_tolerance):
                                    match_found = True
                                    print(f"    ✅ Match found at offset ({col-search_radius}, {row-search_radius})! Color: {current_pixel_rgb}")
                                    break
                                
                                # Track closest color for debugging
                                diff = abs(current_pixel_rgb[0] - self.store_fruit_color[0]) + \
                                       abs(current_pixel_rgb[1] - self.store_fruit_color[1]) + \
                                       abs(current_pixel_rgb[2] - self.store_fruit_color[2])
                                if diff < min_diff:
                                    min_diff = diff
                                    closest_color = current_pixel_rgb
                        ...
                        if match_found:
                            ...
                            print(f"    ❌ No match in 5x5 area. Target: {self.store_fruit_color}")
                            print(f"       Closest found: {closest_color} (Diff: {min_diff})")
                            print("       Try re-capturing the color or increasing tolerance.")
                else:
                     print("  ⚠️ No Store Fruit Color set - assuming fruit is present (Legacy Mode).")
                     fruit_detected = True
                
                # If fruit detected, send webhook and store
                if fruit_detected:
                     # Send Discord Webhook (if URL set)
                    if self.discord_webhook_url:
                         print("  📸 Sending Discord Notification...")
                         # Capture full screen for context
                         screenshot_path = os.path.join(os.getcwd(), f"fruit_found_{int(time.time())}.png")
                         try:
                             with mss.mss() as sct:
                             ...
                             # Replace \n with actual newlines if typed as literal \n in Entry
                             formatted_msg = self.discord_message.replace(r'\n', '\n')
                             
                             threading.Thread(target=self.send_discord_webhook, 
                                            args=(formatted_msg, screenshot_path)).start()
                         except Exception as e:
                             print(f"  ❌ Failed to prepare webhook: {e}")
                    ...
                    print("=== Auto Store Devil Fruit: Skipped (No fruit) ===")
    ...
    def update_discord_settings(self):
        """Update Discord Webhook URL and Message"""
        self.discord_webhook_url = self.webhook_url_var.get()
        self.discord_message = self.discord_msg_var.get()
        self.save_settings()
        print(f"Discord settings updated: URL={self.discord_webhook_url[:10]}..., Msg={self.discord_message}")

    def test_discord_webhook(self):
        """Send a test message to the configured webhook"""
        if not self.discord_webhook_url:
            messagebox.showwarning("Missing URL", "Please enter a Discord Webhook URL first.")
            return
        
        # Run in thread to not freeze GUI
        threading.Thread(target=self.send_discord_webhook, 
                        args=(f"✅ **Test Message**\n{self.discord_message}",)).start()

    def update_periodic_settings(self):
        """Update periodic screenshot settings"""
        self.periodic_screenshot_enabled = self.periodic_enabled_var.get()
        self.periodic_screenshot_interval = self.periodic_interval_var.get()
        self.save_settings()
        print(f"Periodic screenshots updated: Enabled={self.periodic_screenshot_enabled}, Interval={self.periodic_screenshot_interval}m")

    def periodic_screenshot_checker(self):
        """Background thread to send periodic screenshots"""
        print("🕒 Periodic screenshot thread started")
        ...
        while self.is_running:
            try:
                if self.periodic_screenshot_enabled:
                    current_time = time.time()
                    interval_seconds = self.periodic_screenshot_interval * 60
                    
                    if current_time - last_sent_time >= interval_seconds:
                        print(f"🕒 Time for periodic screenshot! (Interval: {self.periodic_screenshot_interval}m)")
                        
                        # Take full screenshot
                        screenshot_path = os.path.join(os.getcwd(), f"status_{int(current_time)}.png")
                        with mss.mss() as sct:
                            sct.shot(output=screenshot_path)
                        ...
                        threading.Thread(target=self.send_discord_webhook, 
                                       args=("Status Check", screenshot_path)).start()
                        
                        last_sent_time = current_time
                
                # Sleep to prevent high CPU (check every 5 seconds)
                time.sleep(5)
            except Exception as e:
                print(f"Error in periodic screenshot thread: {e}")
                time.sleep(10) # Wait longer on error
                
        print("🕒 Periodic screenshot thread stopped")

    def capture_store_fruit_color_ui(self):
        """Allow user to click to capture the store fruit button color"""
        self.capture_color_button.config(state="disabled", text="Click Fruit Button...")
            ...
            if pressed:
                ...
                try:
                    with mss.mss() as sct:
                        ...
                        pixel = img[0, 0]
                        # Store as [R, G, B]
                        color = [int(pixel[2]), int(pixel[1]), int(pixel[0])]
                        
                        self.store_fruit_color = color
                        self.store_fruit_color_label.config(text=f"RGB: {color}", foreground="green")
                        self.save_settings()
                        print(f"Store Fruit Color captured at ({x}, {y}): {color}")
                except Exception as e:
                    print(f"Error capturing color: {e}")
                
                self.capture_color_button.config(state="normal", text="Capture Fruit Color")
                return False
    ...
    def send_discord_webhook(self, message, screenshot_path=None):
        """Send message and optional screenshot to Discord Webhook"""
        ...
        if screenshot_path and os.path.exists(screenshot_path):
            try:
                files = {
                    "file": (os.path.basename(screenshot_path), open(screenshot_path, "rb"))
                }
            except Exception as e:
                print(f"Error opening screenshot: {e}")
        
        try:
            response = requests.post(self.discord_webhook_url, data=data, files=files)
